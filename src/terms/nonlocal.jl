...
function (::AtomicNonlocal)(basis::PlaneWaveBasis{T}) where {T}
    ...
    batch_size = length(model.atoms)
    if !isnothing(basis.architecture.nonlocal_batch_size)
        batch_size = basis.architecture.nonlocal_batch_size
    end
    ops = map(basis.kpoints) do kpt
        form_factors = build_group_form_factors(basis, kpt, psps)
        coupling_terms = build_group_coupling_terms(basis, psps)
        BatchedNonlocalOperator(basis, kpt, form_factors, coupling_terms, batch_size)
    end
    TermAtomicNonlocal(ops)
end

struct TermAtomicNonlocal <: TermLinear
    ops::Vector{BatchedNonlocalOperator}
end
...
@timing "ene_ops: nonlocal" function ene_ops(term::TermAtomicNonlocal,
    ...
    for (ik, ψk) in enumerate(ψ)
        batched_application(term.ops[ik]) do P, D
            Pψk = P' * ψk # nproj x nband
            band_enes = dropdims(sum(real.(conj.(Pψk) .* (D * Pψk)), dims=1), dims=1)
...
@timing "forces: nonlocal" function compute_forces(term::TermAtomicNonlocal,
    ...
    # Energy terms are of the form <ψ, P D P' ψ>, where
    ...
    for (ik, kpt) in enumerate(basis.kpoints)
        ...
        for (igroup, group) in enumerate(psp_groups)
            ...
            D = term.ops[ik].coupling_terms[igroup]
...
function build_group_coupling_terms(basis::PlaneWaveBasis{T},
                                    psps::AbstractVector{<: NormConservingPsp}) where {T}
    coupling_terms = Vector{AbstractMatrix{T}}()
    for psp in psps
        push!(coupling_terms, to_device(basis.architecture, build_projection_coefficients(T, psp)))
    end 
    coupling_terms 
end

# TODO: get rid of this and only do batching ?
...
# TODO: get rid of this and only do batching ? (and adapt docs, ofc)
@doc raw"""
